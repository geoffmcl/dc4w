...
DWORD dir_getpathsize(LPSTR path);
...
BOOL dir_copy(DIRITEM item, LPSTR newroot)
{
        ...
        pstart = relname;
        while ( (pel = (LPSTR)strchr((unsigned char*)pstart, '\\')) != NULL) {
                 ...
                 * ignore .
                 */
                if (strncmp((unsigned char*)pstart, (unsigned char*)".\\", 2) != 0) {

                        pdest = &newpath[lstrlen(newpath)];
                        strncpy((unsigned char*)pdest, (unsigned char*)pstart, pel - pstart);
                        pdest[pel - pstart] = '\0';
...
dir_scan(DIRECT dir, BOOL bRecurse)
{
        ...
        path = (PSTR)LocalLock(LocalAlloc(LHND, size));
        ...
        if (bRecurse) {
                List_TRAVERSE_D(dir->directs, child) {
                        dir_scan(child, TRUE);
                }
        }
...
dir_addfile(DIRECT dir, LPSTR path, DWORD size)
{
    DIRITEM pfile;
        ...
        //List_TRAVERSE_M(dir->diritems, pfile, DIRITEM);
        for(pfile = (DIRITEM)List_First(dir->diritems) ;  pfile != NULL ;  pfile = (DIRITEM)List_Next((LPVOID)pfile) )
        {
                ...
                if (utils_CompPath(pfile->name, path) > 0) {
                        ...
                        pfile = (DIRITEM)List_NewBefore(dir->diritems, pfile, sizeof(struct diritem));
                        dir_fileinit(pfile, dir, path, size);
                        return;
                }
        }
        /* goes at end */
        pfile = (DIRITEM)List_NewLast(dir->diritems, sizeof(struct diritem));
        dir_fileinit(pfile, dir, path, size);
} /* dir_addfile */
...
dir_adddirect(DIRECT dir, LPSTR path)
{
        ...
        //for (child = (DIRECT)List_First(dir->diritems); child != NULL; child = (DIRECT)List_Next((LPVOID)child))
        List_TRAVERSE_M(dir->directs, child, DIRECT)
        {
                 ...
                 */
                strncpy((unsigned char*)achTempName, (unsigned char*)finalel, lstrlen(finalel)-1);
...
dir_finalelem(LPSTR path)
{
        ...
        chp = (LPSTR)strrchr((unsigned char*)path, '\\');
        if (chp != NULL) {
                return(chp+1);
        }
...
dir_getpathsize(LPSTR path)
{
        ...
        size = GetFileSize( (HANDLE)fh, NULL); // TODO: fix type cast warning!!!
        _lclose(fh);
        return(size);
} /* dir_getpathsize */
