...
HWND hwndStatus = 0;        /* status bar across top */
...
#ifdef GUTILS_STATIC
        if (!DllMain(hInstance, DLL_PROCESS_ATTACH, 0))
            return(FALSE);
#endif // GUTILS_STATIC
...
SetNames(LPSTR names)
{
        ...
        if (names == NULL) {
                ...
                strncpy((unsigned char *)AppTitle, (unsigned char*)names, sizeof(AppTitle));
        }
}
        ...
        for (i = start; i < endcell; i++) {
                ...
                if (cd->nchars != 0) {
                        if (textp == NULL) {
                                ...
                                strncpy((unsigned char*)cd->ptext, (unsigned char *)textp, cd->nchars -1);
...
#if 0 // 00000000000 already in gutils lib 00000000000000
/***************************************************************************
 * Function: My_mbschr
 *
 * Purpose:
    ...
    unsigned char *psz, unsigned short uiSep)
{
    ...
    while (psz > pszHead) {
        psz = (unsigned char* )CharPrev((LPCSTR)pszHead, (LPCSTR)psz);
        if (*psz == uiSep) {
            break;
        }
    }
