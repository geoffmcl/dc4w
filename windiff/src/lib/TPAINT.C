...
void gtab_delcr(LPSTR ptext)
{
        ...
        for(chp = ptext; (chp = (LPSTR)strchr((unsigned char*)chp, '\r')) != NULL; ) {
                *chp = ' ';
        }
        for(chp = ptext; (chp = (LPSTR)strchr((unsigned char*)chp, '\n')) != NULL; ) {
                *chp = ' ';
        }
}
...
gtab_updatecontig(HWND hwnd, lpTable ptab, int line, int cell1, int count)
{
        ...
        if (list.row < ptab->hdr.nrows) {
                gtab_sendtq(hwnd, TQ_GETDATA, (LPARAM)(LPSTR) &list);
        }
...
gtab_paintcell(HWND hwnd, HDC hdc, lpTable ptab, int line, int cell)
{
        ...
        if (cd->props.valid & P_FONT) {
                hfont = (HFONT)SelectObject(hdc, cd->props.hFont);
        ...
        for (chp = cd->ptext;
            ((chp != NULL) && ((tabp = (LPSTR)strchr((unsigned char*)chp, '\t')) != NULL)); ) {
