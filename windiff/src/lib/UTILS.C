 ...
 */
int FAR PASCAL dodlg_stringin(HWND hDlg, UINT message, UINT wParam, LPARAM lParam);
...
readfile_next(FILEBUFFER fbuf, int FAR * plen)
{
         ...
         */
        Old_strncpy((unsigned char*)fbuf->buffer, (unsigned char*)fbuf->start, fbuf->last - fbuf->start);
        fbuf->last = &fbuf->buffer[fbuf->last - fbuf->start];
        fbuf->start = fbuf->buffer;

        /* read in to fill the block */
        fbuf->last += _lread(fbuf->fh, fbuf->last,
            (UINT)(&fbuf->buffer[sizeof(fbuf->buffer)] - fbuf->last));
...
utils_CompPath(LPSTR left, LPSTR right)
{
        ...
        */
        ...
        left = (LPSTR)strchr((unsigned char*)left, '\\');
        right = (LPSTR)strchr((unsigned char*)right, '\\');
 ...
 */
int FAR PASCAL
dodlg_stringin(HWND hDlg, UINT message, UINT wParam, LPARAM lParam)
{
        switch(message) {
    ...
    unsigned char *psz, unsigned short uiSep)
{
    while (*psz != '\0' && *psz != uiSep) {
        psz = (unsigned char*)CharNext((LPCSTR)psz);
    }
