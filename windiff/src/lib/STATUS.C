 ...
 */
BOOL
StatusInit(HINSTANCE hInstance)
{
        WNDCLASS    wc;
        BOOL resp = FALSE;
        TEXTMETRIC tm;
 ...
 */
HWND APIENTRY
StatusCreate(HINSTANCE hInst, HWND hParent, int id, LPRECT rcp, HANDLE hmem)
{
...
StatusWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        hitems = (HANDLE) (LONG_PTR) cp->lpCreateParams;
        ...
        if (plist->selitem != -1) {
                ...
                if (PtInRect(&ip->rc, pt)) {
                        SendMessage(GetParent(hWnd), WM_COMMAND,
                                MAKELONG(ip->id, WM_LBUTTONUP), (LPARAM)hWnd);
                }
        }
        GlobalUnlock(hitems);
        break;
    ...
    case SM_SETTEXT:
        ...
        if (ip != NULL) {
                if (lParam == 0) {
                        ...
                        strncpy((unsigned char*)ip->text, (unsigned char*)lParam, SF_MAXLABEL);
