...
void APIENTRY List_Init(void)
  {  LIST P;
     ...
     iAnchorSize = (int)((char FAR *)&(P->iLen) - (char FAR *)&(P->pitNext));
     iHeaderSize = (int)((char FAR *)&(P->Data) - (char FAR *)&(P->pitNext));
...
LIST APIENTRY List_Create(void)
  ...
  {  LIST pit;      /* newly allocated item */
     ...
     if (lst==NULL)
       return;
     pit = (LIST)Alloc(iHeaderSize+uLen);
  ...
  {  LIST pitNew;
     ...
     if (Curs==NULL){ List_AddFirst(lst, pObject, uLen);}
        ...
        {  MOVEBACK(Curs);
           pitAfter = (LIST)Curs;
           pitNew = (LIST)Alloc(iHeaderSize+uLen);
