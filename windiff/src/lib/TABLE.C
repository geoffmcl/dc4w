...
void gtab_append(HWND hwnd, lpTable ptab, int rows, LONG_PTR id);
 ...
 */
 LRESULT CALLBACK gtab_wndproc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
        ...
        case WM_CREATE:
                ...
                if (csp->lpCreateParams == NULL) {
                        ...
                        hOwner = (HWND) (LPARAM) csp->lpCreateParams;
                }
                ptab = NULL;
                hHeap = gmem_init();
        ...
        case TM_APPEND:
                ...
                ptab = (lpTable) GetWindowLongPtr(hwnd, WL_TABLE);
                if (ptab != NULL) {
                        gtab_append(hwnd, ptab, (int)wParam, lParam);
                        return(TRUE);
                }
                break;
 ...
 */
lpTable
gtab_buildtable(HWND hwnd, LONG_PTR id)
{
        lpTable ptab;
        HANDLE hHeap;
        int ncols, i;
        ...
        if (gtab_sendtq(hwnd, TQ_GETSIZE, (LPARAM) (LPSTR)&ptab->hdr) == FALSE) {
                return(NULL);
        }
        ...
        cplist.startcol = 0;
        cplist.ncols = ncols;
        gtab_sendtq(hwnd, TQ_GETCOLPROPS, (LPARAM) (LPSTR)&cplist);
...
gtab_newsize(HWND hwnd, lpTable ptab)
{
        ...
        /* get new row count */
        hdr = ptab->hdr;
        gtab_sendtq(hwnd, TQ_GETSIZE, (LPARAM) (LPSTR) &hdr);
 ...
 */
void
gtab_append(HWND hwnd, lpTable ptab, int rows, LONG_PTR id)
{
        long range;
        long oldrows;
        int line, nupdates;
