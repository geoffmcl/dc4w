...
tree_delitem(TREEITEM item)
{
        ...
        if (item->data != NULL) {
                gmem_free(item->root->hHeap, (LPSTR)item->data, item->length);
        }
...
tree_update(TREE tree, TREEKEY key, LPVOID value, UINT length)
{
        ...
        if (item->key == key) {
                ...
                if (length != item->length) {
                        gmem_free(tree->hHeap, (LPSTR)item->data, item->length);
...
ctree_update(TREE tree, TREEKEY key, LPVOID value, UINT length)
{
        ...
        pcounter = (long *)tree_search(tree, key, &item);
        ...
        if (pcounter == NULL) {
                 ...
                 * the beginning
                 */
                pcounter = (long *)tree_addafter(tree, &item, key, NULL,
                                length + sizeof(long));
                *pcounter = 1;
...
ctree_getcount(TREE tree, TREEKEY key)
{
        ...
        pcounter = (long *)tree_find(tree, key);
        if (pcounter == NULL) {
                return(0);
        }
        return(*pcounter);
