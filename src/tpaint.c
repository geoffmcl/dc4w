 ...
 * Paints a cell.
 *
 * Can be to a PRINTER or SCREEN HDC after calling gtab_updateline();
 *
 */
void
gtab_paintcell(HWND hwnd, HDC hdc, lpTable ptab, int line, int cell)
{
        ...
        // the actual text data is
        //chp = cd->ptext;
        // NOTE: can be NULL if NO data for cell/column in the line/row
        ...
        if (cd->props.valid & P_FONT) {
                ...
                tabwidth = tm.tmAveCharWidth * giTabSize;
        } else {
                //tabwidth = ptab->avewidth * 8;
                tabwidth = ptab->avewidth * giTabSize;
        }
        ...
        if (align == P_LEFT) {
                ...
                if (align == P_CENTRE) {
                        cx = (ppos->px_size - cx) / 2;
                } else {
                        cx = ppos->px_size - cx - (ptab->avewidth/2);
                }
        }
        cx += ppos->start;
        ...
        if (cd->props.valid & P_BOX)
        {
                if (cd->props.box != 0)
                {
                        ...
                        rcbox.bottom = rcbox.top + pline->linepos.px_size;
                        rcbox.left = ppos->start;
                        rcbox.right = ppos->start + ppos->px_size;
 ...
 *
 * Can be to a PRINTER or SCREEN HDC
 *
 */
void
...
gtab_vsep(HWND hwnd, lpTable ptab, HDC hdc)
{
        ...
        // paint VERTICAL as THREE (3) pixels wide
        if(x)
        {
           x2 = x - 1;
           MoveToEx(hdc, x2, rc.top, NULL);
...
void gtab_drawhorzline(HWND hwnd, INT y)
{
   RECT rc;
   HDC hdc;
   HPEN hpen;
