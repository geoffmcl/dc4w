...
typedef struct {
        LONG_PTR id;               /* caller's id for data */
...
typedef struct {
        ...
        long selrow;    // KEEP selected ROW number, 0 based
...
typedef struct {
        ...
        int px_size;            /* pixel size of cell (width or height) */
...
typedef struct {
        LONG_PTR id;               /* id of table to print */
        lpTitle head;
        lpTitle foot;
        lpMargin margin;
        PRINTDLG FAR * pd;
