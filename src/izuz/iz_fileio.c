...
#  endif
#endif
#include "iz_ebcdic.h"   /* definition/initialization of ebcdic[] */
...
#if   (defined(IZDLL) && defined(ADDSR))
...
/* GRR:  need to change this to *q++ = native(*p++); loop or something */
    ...
    /*-----------------------------------------------------------------------
      ...
      -----------------------------------------------------------------------*/
        ...
        /* else not VMS text */ {
            p = rawbuf;
            if (*p == mLF && G.didCRlast)
                ++p;
            G.didCRlast = FALSE;
            for (q = transbuf;  p < rawbuf+(unsigned)size;  ++p) {
                if (*p == mCR) {           /* lone CR or CR/LF: treat as EOL  */
                    PutNativeEOL
                    if (p == rawbuf+(unsigned)size-1) /* last char in buffer */
                        G.didCRlast = TRUE;
                    else if (p[1] == mLF)  /* get rid of accompanying LF */
                        ++p;
                } else if (*p == mLF)      /* lone LF */
                    PutNativeEOL
                else
...
#endif
 */
#ifdef IZDLL                 /* don't display message if data is redirected */
            ...
            while (*++p) {
                ...
                if (*p == 0x1B) {          /* ASCII escape char */
                    ...
                    *q++ = '[';
                    ...
                    if (p[1] == mLF)        /* ASCII LF */
                        *q++ = *++p;
                    else if (p[1] == mCR && p[2] == mLF) {  /* ASCII CR LF */
                        *q++ = *++p;
                        *q++ = *++p;
                    }
                } else
