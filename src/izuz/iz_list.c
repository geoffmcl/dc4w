...
#  endif
   ...
   static ZCONST char Far HeadersL1[] =
     "--------  ------  ------- -----   ----   ----   ------    ----";
   ZCONST char Far *Headers[][2] =
     { {HeadersS, HeadersS1}, {HeadersL, HeadersL1} };

   ZCONST char Far CaseConversion[] =
     "%s (\"^\" ==> case\n%s   conversion)\n";
...
#ifdef   GMUZ
extern   int store_info( __GDEF );   /* return 0 if skipping, 1 if OK */
#endif   // GMUZ    
...
//#ifndef WINDLL
#if   !(defined(WINDLL) || defined(IZDLL))
...
#ifdef   GMUZ
            store_info(__G);   /* return 0 if skipping, 1 if OK */
#endif   // GMUZ
#ifdef OS2DLL
...
#ifndef  GMUZ     // don't fizzle with the date/time values
...
//#ifdef WINDLL
...
#ifdef   GMUZ
            {
               ...
               else
               {
                  ...
                  ss = (((unsigned)(G.crec.last_mod_dos_datetime)       & 0x1f) * 2   );
               }

               (*G.lpUserFunctions->SendApplicationMessage)(G.crec.ucsize, csiz, (unsigned)cfactor,
                  mo, dy, yr, hh, mm, ss,
                 (LPSTR)fnfilter(G.filename, slide),
                 dos_to_unix_time(G.crec.last_mod_dos_datetime),
                 G.pInfo->file_attr );
            }
#else    // !GMUZ
...
//#ifdef WINDLL
    ...
    {
        cfactor = ratio(tot_ucsize, tot_csize);
        /* pass the totals back to the calling application */
        G.lpUserFunctions->TotalSizeComp = tot_csize;
        G.lpUserFunctions->TotalSize     = tot_ucsize;
        G.lpUserFunctions->CompFactor    = (ulg)cfactor;
