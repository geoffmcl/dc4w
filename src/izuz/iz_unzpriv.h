...
#ifndef MAIN
#ifdef   _MSC_VER
#  define MAIN  Uz_Main   /* was UzpUnzip */
#else // !_MSC_VER
#  define MAIN  main
...
#define mCR     13        /* '\r' on ASCII machines; must be 13 due to EBCDIC */
...
#if (defined(IZDLL) && !defined(NO_SLIDE_REDIR))
...
#    define _ZLS_RELOAD(abs_offset) {\
         MTrace((stderr, "ZLSEEK: %cbs_offset = %ld, G.extra_bytes = %ld\n",\
           'a', (abs_offset), G.extra_bytes));\
         G.cur_zipfile_bufstart = (int)lseek(G.zipfd,(LONGINT)bufstart,SEEK_SET);\
         MTrace((stderr,\
           "       request = %ld, (abs+extra) = %ld, inbuf_offset = %ld\n",\
           request, ((abs_offset)+G.extra_bytes), inbuf_offset));\
         MTrace((stderr, "       bufstart = %ld, cur_zipfile_bufstart = %ld\n",\
