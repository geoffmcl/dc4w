...
/*---------------------------------------------------------------------------
  ...
  You must define `IZDLL' in order to include the API extensions.
  This can be because you ARE doing a DLL form, or simply
  that you WANT to redirect the OUTPUT to ALLOCATED MEMORY
...
#endif
#include "iz_version.h"

#ifndef  GMUZ
// =======================
#ifdef IZDLL      /* This source file supplies DLL-only interface code. */
...
#ifndef WINDLL
#ifdef  GMUZ
//static void WINAPI DummySendMsg(ulg dwSize, ulg dwCompressedSize, unsigned ratio,
...
int UZ_EXP UzpUnzipToMemory(char *zip, char *file, UzpOpts *optflgs,
    UzpCB *UsrFuncts, UzpBuffer *retstr)
{
   //static USERFUNCTIONS  _ssUF;
   int   i = 0;
//   CONSTRUCTGLOBALS();
//   G.lpUserFunctions = &_ssUF;
   // NOTE: This is NOT STRICTLY correct. Should directly call gmuzMemory()
   // *********************************************************************
//   if( UsrFuncts->msgfn )
//      G.lpUserFunctions->SendApplicationMessage = (APPMESSAGE *)UsrFuncts->msgfn;
//   else
//      G.lpUserFunctions->SendApplicationMessage = &DummySendMsg;

//   i = gmuzToMemory( __G__ zip, file, retstr );
   return i;
}
     ...
     __GDEF
{
    if (G.pInfo->textmode) {
        *G.redirect_pointer = '\0';
        G.redirect_size = (unsigned int)(G.redirect_pointer - G.redirect_buffer);
...
#ifdef  GMUZ
int gmuzToMemory(__GPRO__ char * zip, char *file, UzpBuffer *retstr)
{
    int r;
    char *incname[2];
    ...
    G.extract_flag = TRUE;
    uO.qflag = 2;
    uO.vflag = 0;  // kill listing
    G.wildzipfn = zip;   // done above
