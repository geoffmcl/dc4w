...
#define  fg_BlankLine      0x00000001  // we just have spaces
...
struct section {
        ...
        int s_state;              /* compare state for section */
        DWORD  s_dwFlag;   // finer tuning flags

        // compare counts
        DWORD  s_dwSmCnt;  // left line found same in right
        DWORD  s_dwLfCnt;  // only exists in left
        DWORD  s_dwRtCnt;  // only exists in right
        DWORD  s_dwMRCnt;  // is right moved line count
        DWORD  s_dwMLCnt;  // is left moved line count
...
LSTSTATS sLineStats;
PLSTSTATS   pLnStats = &sLineStats;
INT         g_iFD = 0;

typedef struct tagLNSTATS {
   DWORD dwlsid;  // id of the COMPITEM
   DWORD same;
   DWORD left;
   DWORD right;
   DWORD movl;
...
LNSTATS  g_LineStats;
PLNSTATS g_pLineStats = &g_LineStats;
PLSTSTATS   g_pListStats = &g_LineStats.sListStats;

#define  ls    (*g_pLineStats)
#define  lss   (*g_pListStats)

// actual 'lines' of data in left and right files
#define  g_LnsLeft   lss.dwleftcnt
#define  g_LnsRight  lss.dwrightcnt

#define  g_LnTotal   lss.dwtot      // total SECTIONS of lines processed
// while comparing two files of data
#define  g_LnsSame   lss.dwsame     // total SECTIONS that are the 'same'
// if a link was found further down then it has 'moved'
#define  g_LnMovL    lss.dwnewer    // left -> right
#define  g_LnMovR    lss.dwolder    // right -> left
// and lones ONLY found in left or right
#define  g_LnLeft    lss.dwleft
#define  g_LnRight   lss.dwright
// for the files to be "equal", except for date perhaps, then at least
#define  g_FilesEqual    (( lss.dwid ) && \
 ( g_LnsLeft ) && ( g_LnsLeft == g_LnsRight ) )

#ifdef   ADDSIMPSTS2
// not presently used
#define g_dwlnsame   ls.same
#define g_dwlnleft   ls.left
#define g_dwlnright  ls.right
#define g_dwlnmovedleft ls.movl
#define g_dwlnmovedright ls.movr
#define g_dwlnunknown ls.unk
#endif   // #ifdef   ADDSIMPSTS2

VOID  ShowLineStats( VOID )
{
   if( g_iFD )
   {
      ...
      *lpb = 0;
      ...
      if(( g_LnsLeft ) &&
         ( g_LnsLeft == g_LnsRight ) )
      {
         // we have LINES, and they are equal
         sprintf(EndBuf(lpb), "BLines=%d ", g_LnsLeft );
      }
      else if( g_LnsLeft || g_LnsRight )
      {
         if( g_iFD == 1 )
            sprintf(EndBuf(lpb), "Lines = %d ", g_LnsLeft );
         else if( g_iFD == 2 )
            sprintf(EndBuf(lpb), "Lines = %d ", g_LnsRight );
         else  // if( g_iFD == 3 )
            sprintf(EndBuf(lpb), "Lines LR=%d.%d ", g_LnsLeft, g_LnsRight );
      }
      else
      {
         strcpy(lpb,"Ln Totals = ZERO. ");
      }

      if( g_LnTotal || g_LnsSame || g_LnMovL || g_LnMovR )
      {
         if(( g_iFD == 1 ) ||
            ( g_iFD == 2 ) )
         {
            // no 'moved' lines if just left or right full section of lines
            sprintf(EndBuf(lpb), "T=%d S=%d ",
               g_LnTotal,
               g_LnsSame );
         }
         else
         {
            if(( g_FilesEqual ) &&
                  ( g_LnTotal == 1 ) &&   // only ONE section
                  ( g_LnsSame == 1 ) &&    // of at least one line
                  ( g_LnMovL == 0  ) &&
                  ( g_LnMovR == 0  ) )
            {
               sprintf(EndBuf(lpb), "T=%d S=%d ",
            ...
            {
               sprintf(EndBuf(lpb), "T=%d S=%d ML=%d MR=%d ",
                  g_LnTotal,
                  g_LnsSame,
                  g_LnMovL,
                  g_LnMovR );
...
VOID  section_setcompstats(DWORD dwLeft, DWORD dwRight, INT iFD )
{
   g_iFD = iFD;
   if( iFD == 3 ) // passing both
   {
      g_LnsLeft = dwLeft;
      g_LnsRight = dwRight;
   }
...
VOID  ZeroLineStats( VOID )
{
   ZeroMemory( g_pLineStats, sizeof(LNSTATS) );
}

VOID  ResetLineStats( LIST compo )
{
   if( lss.dwid != (LONG_PTR)compo )
   {
        ZeroLineStats();
        lss.dwid = (LONG_PTR)compo;    // set stat ID
   }
}

VOID  SimpleLineStats( INT state )
{
#ifdef   ADDSIMPSTS2
   switch(state)
   {
 ...
 * on the list. otherwise we will alloc it from gmem get(hHeap). We remember
 * this in the bDiscard flag for section_delete, so that we only
 * hand back to gmem free memory that we got.
 *
 * NOTE: This SEEMS reversed. If it is allocated on a LIST, then
 *    say List_NewLast() has it OWN LAlloc-ate and LFree-ing of memory.
 * SO, it seems the bDiscard SHOULD be only SET when allocated from
 *    our global HEAP handler!!!!!!!!!!!!
 * TRY IT AND SEE?!?!?!?!?
 * As the section_delete says
...
SECTION section_new(LINE first, LINE last, LIST list)
{
        ...
        if (list)
        {
                ...
                //sec->bDiscard = TRUE;
                sec->bDiscard = FALSE; // no discard - memory freed when LIST killed
        }
        else
        {
                sec = (SECTION) gmem_get(hHeap, sizeof(struct section), "section_new" );
                //sec->bDiscard = FALSE;
                sec->bDiscard = TRUE;  // allocated so set the DISCARD flag
        }
...
void section_delete(SECTION section)
{
   ...
   if( section->bDiscard )
   {
      gmem_free(hHeap, (LPSTR) section, sizeof(struct section), "section_new" );
   }
}
 ...
 */
BOOL section_match(SECTION sec1, SECTION sec2)
{
        TREE ctleft, ctright;
        LINE line, line2;
        BOOL bLinked = FALSE;
...
int section_getstate(SECTION section)
{
        if (section == NULL) 
                return(0);
        return(section->s_state);
}

DWORD section_getflag(SECTION section)
{
        if (section == NULL) 
                return(0);
        return(section->s_dwFlag);
}
...
{
   if(section)
      section->s_state = state;
}

void section_setflag(SECTION section, DWORD dwFlag)
{
   if(section)
      section->s_dwFlag = dwFlag;
}
...
void section_setleftbasenr(SECTION section, int base)
{
...
void section_setrightbasenr(SECTION section, int base)
{
...
LIST section_makelist(LIST linelist, BOOL left)
{
     ...
     List_TRAVERSE(linelist, line1)
     {
           ...
           blank = line_isblank(line1);   // set this 'important' condition
         ...
         // ( ignore_blanks && line_isblank(line1) ) )
          ...
          //sect->s_state = (matched ? STATE_SAME
          //section_setstate(sect, ( matched ? STATE_SAME
          //                         : left ? STATE_LEFTONLY
          //                              : STATE_RIGHTONLY ) );
          state = ( matched ? STATE_SAME
                              : left ? STATE_LEFTONLY : STATE_RIGHTONLY );

          if(sect) { // FIX20041218 - uninit'ed 'sect' in section.c

             if( blank )
               sect->s_dwFlag |= fg_BlankLine;   // set section as having a BLANK line
...
BOOL section_matchlists(LIST secsleft, LIST secsright)
{
        ...
        */
        ...
        List_TRAVERSE(secsleft, sec1)
        {
                ...
                if (secTemp && secTemp->s_state!= STATE_SAME)
                {
                        continue;
                }
 ...
 *
 */
void section_takesection(LIST compo, SECTION left, SECTION right, int state)
{
        SECTION newsec;
        SECTION sec;

        SimpleLineStats( state );   // update stats
        ...
        case STATE_SAME:
                 ...
                 */
                if (left!=NULL) left->s_state = STATE_MARKED;
                right->s_state = STATE_MARKED;
                sec = right;
                break;
...
LIST section_makecomposite(LIST secsleft, LIST secsright)
{
        ...
        left  = List_First(secsleft);   // get first LEFT line, and
        right = List_First(secsright); // first RIGHT line

        ResetLineStats( compo );

        // while left and right lines exist...
           ...
           g_LnTotal++;
                if (left == NULL)
                {
                        ...
                        if (right->link == NULL)
                        {
                                ...
                                g_LnRight++;  // bump right only sections
                        }
                        else
                        {
                ...
                else if (right == NULL)
                {
                        ...
                        if (left->link == NULL)
                        {
                                ...
                                left = List_Next(left);
                                g_LnLeft++;  // bump LEFT section count
                        }
                        else
                        {
                                section_takesection(compo, left, left->link, STATE_MOVEDLEFT);
                                left = List_Next(left);
                                g_LnMovL++;  // or moved section
                        }

                }
                else if (left->s_state == STATE_LEFTONLY)
                {
                ...
                else if (left->link->s_state==STATE_MARKED)
                {
                        /* left is linked to section that is already taken*/
                        section_takesection(compo, left, left->link, STATE_MOVEDLEFT);
                        left = List_Next(left);
                        g_LnMovL++; // bump section count
                }
                else  if (right->link == NULL)
                {
                         ...
                         */
                        section_takesection(compo, NULL, right, right->s_state);
                        right = List_Next(right);
                        g_LnRight++;
                }
                else if (right->link->s_state==STATE_MARKED)
                {
                ...
                {
                   /* sections match
                    * lines are equal (using current compare rules) 
                    * just move on in both line lists 
                    */
...
BOOL section_expandanchor(SECTION sec1, LINE line1, SECTION sec2, LINE line2)
{
        ...
        for (; ; )
        {
                if (line_link(left, right) ) { // compare lines - has and actual comp
                        bChanges = TRUE;
                        left = List_Next(left);
                        right = List_Next(right);
...
// newer
LIST section_makeone(LIST file)
{
   SECTION lines;
//   LIST compo;
...
//   compo = List_Create();
   ...
   lines  = List_First(file);   // get lines

   ResetLineStats( file );

   // while left and right lines exist...
   while( lines != NULL ) // || (right != NULL) )
   {
      g_LnTotal++;
      lines = List_Next(lines);
   }
