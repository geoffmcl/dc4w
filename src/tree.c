...
*
* Mmemory is allocated from gmem get, using hHeap as the heap handle.
...
tree_delitem(TREEITEM item)
{
        ...
        if (item->data != NULL) {
           gmem_free(item->root->hHeap, item->data, item->length, "tree_newitem" );
        }

        gmem_free(item->root->hHeap, (LPSTR) item, sizeof(struct treeitem), "tree_newitem1" );
}
...
tree_newitem(TREE root, TREEKEY key, LPVOID value, UINT length)
{
        ...
        item = (TREEITEM) gmem_get(root->hHeap, sizeof(struct treeitem), "tree_newitem1" );
        ...
        item->right = NULL;
        item->length = length;
        item->data = gmem_get(root->hHeap, length, "tree_newitem" );
        if (value != NULL) {
...
tree_create(HANDLE hHeap)
{
        ...
        tree = (TREE) gmem_get(hHeap, sizeof(struct tree), "tree_create" );
        tree->first = NULL;
        tree->hHeap = hHeap;
        return(tree);
}
...
tree_delete(TREE tree)
{
        ...
        gmem_free(tree->hHeap, (LPSTR) tree, sizeof(struct tree), "tree_create" );
}
...
tree_update(TREE tree, TREEKEY key, LPVOID value, UINT length)
{
        ...
        if (item->key == key) {
                ...
                if (length != item->length) {
                   gmem_free(tree->hHeap, item->data, item->length, "tree_newitem" );
                   item->data = gmem_get(tree->hHeap, length, "tree_newitem" );
                }
