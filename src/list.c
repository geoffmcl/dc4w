...
static   TCHAR sszAnchor[] = "anchor_block";
static   TCHAR sszEntry[]  = "list_entry";
 ...
 */
static LPVOID LAlloc(int size, LPTSTR lpn)
{
   HANDLE hMem;
   LPVOID pRet = 0;
...
LA_Exit:
   ...
   // NOTE: This function Enters the critical section.
   // The caller must Leave it with
...
static void LFree(PBLOCK pBlock, LPVOID p, LPTSTR lpn)
{
   HANDLE hMem;
   ...
   if( pBlock->iInUse <= 0 )
   {
      ...
      { 
        ...
        LPTSTR pmsg = &szLBuf[256];
        LoadString(hLibInst, IDS_LIST_ALLOC_NEGATIVE, szLBuf, 256);
        sprintf(pmsg, szLBuf, pBlock->iInUse);
        MB(NULL, pmsg, NULL, MB_OK | MB_ICONSTOP);
      }
      ...
      GlobalFree(hMem);
      if( pCurrent==pBlock )
         pCurrent = NULL; /* defend the invariant */
   }
...
//   sprtf( "List: Init set AnchorSize = %d and HeaderSize = %d."MEOR,
//      iAnchorSize,
//      iHeaderSize );
...
LIST APIENTRY List_Create(void)
{
   ...
   if( !bInited )
   {
      bInited = TRUE;
      List_Init();
   }          /* prevent some silly errors */

   lst = LAlloc(iAnchorSize, sszAnchor);
   if(lst==NULL)
   {
      return NULL;
   }
   ...
   //sprtf( "List: Created list %#x."MEOR, lst );
...
void APIENTRY List_Destroy(PLIST plst)
{
   ...
   do
   {
      pitQ = pitP->pitNext;   // get NEXT in list

      if( pitQ == *plst )  // if next is first, then its the anchor blcok
         LFree(pitP->pBlock, pitP, sszAnchor);
      else                 // else it is just another entry
         LFree(pitP->pBlock, pitP, sszEntry);

      pitP = pitQ;

   }while(pitP != *plst);

   //sprtf( "List: Destroyed list %#x."MEOR, pitP );
...
{  LIST pit;      /* newly allocated item */
   ...
   pit = LAlloc( (iHeaderSize+uLen), sszEntry);
  ...
  {  LIST pitNew;
     ...
     if (Curs==NULL){ List_AddFirst(lst, pObject, uLen);}
        ...
        {  MOVEBACK(Curs);
           ...
           pitNew = LAlloc( (iHeaderSize+uLen), sszEntry );
...
LPVOID APIENTRY List_DeleteForwards(LPVOID Curs)
{
   ...
   LFree(pitDel->pBlock, pitDel, sszEntry);
   if(pitN->bAnchor)
      return NULL;
   else
      return (char FAR *)&(pitN->Data);
...
\******************************************************************************/
// NEW
DWORD APIENTRY List_Count( LIST lst, PDWORD pdwa, PDWORD pdwe )
{
   DWORD dwc = 0;
   DWORD dwa = 0;
   DWORD dwe = 0;
